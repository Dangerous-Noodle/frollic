...
userController.addFavorite = (req, res, next) => {
  ...
  if (!res.locals.authInfo || !res.locals.authInfo.authenticated) {
    return res.status(200).json(createResponse(false, 400, 'You must be logged in to store favorites'));
  } else {
  ...
  if (req.body.businessId) {
    ...
    return res.status(200).json(createResponse(false, 400, 'You must specify a business id'));
  }
...
userController.getFavorites = (req, res, next) => {
  ...
  if (!res.locals.authInfo || !res.locals.authInfo.authenticated) {
    return res.status(200).json(createResponse(false, 400, 'You must be logged in to view favorites'));
  } else {
    userId = res.locals.authInfo.user_id;
  }
